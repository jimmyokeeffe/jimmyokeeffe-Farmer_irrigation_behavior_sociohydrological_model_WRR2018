...
// [[Rcpp::export]]
List gw_model_dailyC_v2(double prec,                     // precipitation (mm)
			...
			bool farmer_canal_access,        // does the current farmer have access to canal water
  ...
  if (farmer_canal_access) {
    ...
    // NB concerned about canal leakage - why divide by farm_area???
    H_new = H_init + recharge_volume + (canal_volume * canal_leakage_coef / farm_area);

  } else {

    if (H_init <= well_depth) {
      ...
      // recharge
      H_new = H_init + recharge_volume;

    } else {

      // otherwise the farmer's well is deep enough to extract groundwater
      H_new = H_init + recharge_volume - irrigation_volume;
    }
  }
  ...
  return List::create(Named("actual_ET") = actual_ET, _["H"] = H_new, _["root_zone_depletion"] = root_zone_depletion_new, _["abstraction"] = irrigation_volume);

}

// [[Rcpp::export]]
List gw_model_dailyC_v1(double prec,                     // precipitation (mm)
			double reference_crop_ET,        // reference crop evapotranspiration (mm)
			double et_depletion_factor,      // ET depletion factor (FAO56)
			double rooting_depth,	      // rooting depth (mm)
			double H_init,                   // groundwater head from previous time point
  ...
  // infiltration to the root zone
  double rain_m = (prec / 1000) * runoff_coef * evaporation_loss_coef * farm_area;
  ...
  if (farmer_canal_access) {
    ...
    H_new = (H_init + rain_m +
             (((farm_area * f1) + (farm_area * f1 * irrigation_eff)) * return_flow_coef) +
             (canal_volume * canal_leakage_coef / farm_area));
    ...
    if (H_init <= well_depth) {
      ...
      // otherwise the farmer's well is deep enough to extract groundwater
      H_new = (H_init -
               ((farm_area * f1) + (farm_area * f1 * irrigation_eff)) + rain_m +
               (((farm_area * f1) + (farm_area * f1 * irrigation_eff)) * return_flow_coef));
    }
  }
  ...
  // abstraction volumne (m3)
  double abstraction_new;
  if (farmer_canal_access) {
    abstraction_new = Rcpp::max(NumericVector::create(0.0, (f1 * farm_area)));
  } else {
    abstraction_new = Rcpp::max(NumericVector::create(0.0, (H_init - H_new)));
  }
  ...
  // double actual_ET = Ks * Kc * reference_ET;
  double actual_ET = Ks * reference_crop_ET;

  // NB abstraction converted to depth (by dividing by farm_area) and
  // units changed to mm
  double root_zone_depletion_new = Rcpp::max(NumericVector::create(0.0, (root_zone_depletion_init + actual_ET) - (prec + ((abstraction_new / farm_area) * 1000))));
   
  return List::create(Named("actual_ET") = actual_ET, _["H"] = H_new, _["root_zone_depletion"] = root_zone_depletion_new, _["abstraction"] = abstraction_new);
...
List model(NumericVector prec,	                 // precipitation, daily
  ...
  for (int i = 0; i < n_day; i++) {
    for (int j = 0; j < n_farmer; j++) {
      ...
      List gw_sim = gw_model_dailyC_v2(prec[i] * rain_rand,                  // precipitation on day i
